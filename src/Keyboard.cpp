...
int Keyboard::gererClavier() {
	...
	keys = SDL_GetKeyStateVita(NULL);
	pollKeys(keys);
    SceTouchData touch;
	sceTouchPeek(SCE_TOUCH_PORT_FRONT, &touch, 1);
    touchLocation.x = touchLocation.y = -1;
	if (touch.reportNum > 0){
		touchLocation.x = touch.report[0].x;
		touchLocation.y = touch.report[0].y;
	}
	return 0;
}
...
int Keyboard::pollKeys(int keys) {
    ...
    if (keys & SCE_CTRL_LTRIGGER) {
		...
		if ( (keys & SCE_CTRL_SQUARE) && mode == 0) {
			mode = 17; 
        ...
        if (keys & 0) return -1;
		if ((keys & SCE_CTRL_CROSS ) && tmpa==0) {
			toggleFullScreen();
			tmpa=1;
		}
    }

    if (!((keys&SCE_CTRL_CROSS) && (keys & SCE_CTRL_LTRIGGER))) tmpa=0; 

    switch (mode) {
        case 0 :
            gpJoueur = gpJeu->getJoueur();

            if ((keys & SCE_CTRL_LTRIGGER) && (keys & SCE_CTRL_START) && tmp == 0) { 
                if (!gpJeu->getStop() && !gpJeu->getMenu()) gpJeu->setMenu(true);
                else if (gpJeu->getMenu()) gpJeu->setMenu(false);
                tmp = 1;
            }
 
            if ((!(keys&SCE_CTRL_START) && !(keys&SCE_CTRL_CROSS) && !gpJeu->getMenu()) || (gpJeu->getMenu() &&
            !(keys&SCE_CTRL_START) && !(keys&SCE_CTRL_CROSS) && !(keys&SCE_CTRL_LEFT) && !(keys&SCE_CTRL_RIGHT) 
            && !(keys&SCE_CTRL_UP) && !(keys&SCE_CTRL_DOWN))) tmp=0;
            
            if (keys&SCE_CTRL_CROSS && !gpJeu->getStop() && gpJoueur->getVie() && 
            (gpJoueur->getTypeAnim()==AUCUNE || gpJoueur->getTypeAnim()==MARCHE) 
             && !gpJoueur->getImmo()  && tmp == 0) {
                gpJeu->lire();
                tmp = 1;
            } else if (!(keys & SCE_CTRL_LTRIGGER) && (keys & SCE_CTRL_CROSS) && tmp == 0) { 
            ...
            if (gpJeu->getText() && gpJeu->getTexte()->isFinished()) {
                if(tmpt==0){
                    ...
                    switch (gpJeu->getTexte()->getId()) {
                        ...
                        case 331 : case 333 : case 360 : case 369 :
                            if ((keys&SCE_CTRL_LEFT) || (keys&SCE_CTRL_RIGHT)) {
                ...
                if (!(keys&SCE_CTRL_CROSS) && !(keys&SCE_CTRL_CROSS) && !(keys&SCE_CTRL_LEFT) 
                && !(keys&SCE_CTRL_RIGHT) && !(keys&SCE_CTRL_UP) && !(keys&SCE_CTRL_DOWN)) tmpt=0;
            }
            
            nbdir=0;
            if (keys&SCE_CTRL_UP) nbdir++;
            if (keys&SCE_CTRL_DOWN) nbdir++;
            if (keys&SCE_CTRL_LEFT) nbdir++;
            if (keys&SCE_CTRL_RIGHT) nbdir++;
           
            int vitesse;
            if (((keys&0) || (keys&SCE_CTRL_RTRIGGER)) && !gpJeu->getStop() 
            ...
            && !gpJeu->getStop() && !gpJeu->getTransition() && !gpJoueur->getImmo()) {
                ...
                if (keys&SCE_CTRL_LEFT) {
                    ...
                    && (!(keys&SCE_CTRL_UP) || gpJoueur->getDirection()!=N) 
                    && (!(keys&SCE_CTRL_DOWN) || gpJoueur->getDirection()!=S))
                ...
                if (keys&SCE_CTRL_UP) {
                    if(!gpJoueur->getCharge()
                    && (!(keys&SCE_CTRL_LEFT) || gpJoueur->getDirection()!=O) 
                    && (!(keys&SCE_CTRL_RIGHT) || gpJoueur->getDirection()!=E))
            ...
            if ((keys&SCE_CTRL_LTRIGGER) && !gpJoueur->getImmo()) {
                if ((keys&SCE_CTRL_LEFT) && gpJeu->getVueHorz()>-64)
                    gpJeu->setVueHorz(gpJeu->getVueHorz()-2);
                if ((keys&SCE_CTRL_RIGHT) && gpJeu->getVueHorz()<64)
                    gpJeu->setVueHorz(gpJeu->getVueHorz()+2);
                if ((keys&SCE_CTRL_UP) && gpJeu->getVueVert()>-64)
                    gpJeu->setVueVert(gpJeu->getVueVert()-2);
                if ((keys&SCE_CTRL_DOWN) && gpJeu->getVueVert()<64)
                    gpJeu->setVueVert(gpJeu->getVueVert()+2);
            }
            ...
            && !gpJoueur->isLapin() && !gpJoueur->getImmo()) {
                if (!(keys & SCE_CTRL_LTRIGGER) && ((keys&SCE_CTRL_CIRCLE) || (keys&0)) && !tmpw) {
                    gpJoueur->setTypeAnim(EPEE);
                    gpJoueur->chargeSpin();
                    tmpw=1;
                }
                if (!(keys&SCE_CTRL_CIRCLE) && !(keys&0) && tmpw) tmpw=0;
            }
            if (!(keys&SCE_CTRL_CIRCLE) && !(keys&0) && gpJoueur->getCharge() && !gpJeu->getStop() 
            && !gpJoueur->isLapin()) {
            ...
            if (!(keys & SCE_CTRL_LTRIGGER) &&(keys&SCE_CTRL_SQUARE) && (
            gpJoueur->getTypeAnim()==AUCUNE || gpJoueur->getTypeAnim()==MARCHE || 
            (gpJoueur->getObjet()==8 && 
            (gpJoueur->getTypeAnim()==PORTE || gpJoueur->getTypeAnim()==EMMENE))) 
            ...
            if (!(keys&SCE_CTRL_SQUARE) && tmpx) tmpx=0;
            
            if (!(keys & SCE_CTRL_LTRIGGER) && (keys&SCE_CTRL_TRIANGLE) && !tmpc && !gpJoueur->getCharge() && gpJoueur->getVie()>0
            ...
            if (!(keys&SCE_CTRL_TRIANGLE) && tmpc) tmpc=0;
            
            if (!(keys & SCE_CTRL_LTRIGGER) && keys&SCE_CTRL_START && (gpJoueur->hasObjet(O_CARTE) || gpJeu->isDonjon()) && 
            ...
            if (!((keys&SCE_CTRL_START)) && tmpp) tmpp=0;
            
 			//open monsters screen
            if ((keys & SCE_CTRL_LTRIGGER) && (keys&SCE_CTRL_CIRCLE) && gpJoueur->hasObjet(O_ENCYCL) 
            ...
            if (!((keys&SCE_CTRL_CIRCLE)) && tmpm) tmpm=0;
            
            if ((keys & SCE_CTRL_LTRIGGER) && keys&SCE_CTRL_TRIANGLE && gpJoueur->hasObjet(O_MASQUE)==2 && !gpJoueur->isLapin() && !tmpo
            ...
            if (!(keys&SCE_CTRL_TRIANGLE) && tmpo) tmpo=0;
            
            
            if ((keys & SCE_CTRL_LTRIGGER) && keys&SCE_CTRL_RTRIGGER && gpJoueur->getOni() && !gpJoueur->isLapin()
            ...
            if (!(keys&SCE_CTRL_RTRIGGER) && tmptp) tmptp=0;
            break;
        case 1 :
            if (!(keys & SCE_CTRL_LTRIGGER) && (keys&SCE_CTRL_CROSS) && tmp == 0) {
                mode = 2;
                gpJeu->getGenerique()->initLogo();
                gpJeu->getAudio()->playSound(1);
                tmp = 1;
            }
            if (!(keys&SCE_CTRL_CROSS)) tmp=0;
            break;
        case 2 :
             if (!(keys & SCE_CTRL_LTRIGGER) && (keys&SCE_CTRL_CROSS) && tmp == 0) {
               mode = 3;
        ...
        case 4 :
            ...
            if ((keys&SCE_CTRL_UP) && !tmp) {
                ligne--; if (ligne<0) ligne=3; tmp=1; 
                if (ligne!=3) colonne=0;
                gpJeu->getAudio()->playSound(3);}
            if ((keys&SCE_CTRL_DOWN) && !tmp) {
                ligne++; if (ligne>3) ligne=0; tmp=1;
                if (ligne!=3) colonne=0;
                gpJeu->getAudio()->playSound(3);}
            if ((keys&SCE_CTRL_LEFT) && !tmp && ligne==3) {
                colonne--; if (colonne<0) colonne=1; tmp=1;
                gpJeu->getAudio()->playSound(3);}
            if ((keys&SCE_CTRL_RIGHT) && !tmp && ligne==3) {
                colonne++; if (colonne>1) colonne=0; tmp=1;
                gpJeu->getAudio()->playSound(3);}
            
            if (!(keys&SCE_CTRL_CROSS) && !(keys&SCE_CTRL_UP) && !(keys&SCE_CTRL_DOWN) 
                && !(keys&SCE_CTRL_LEFT) && !(keys&SCE_CTRL_RIGHT) && tmp) tmp=0;
            break;
        case 6 :
            if (!(keys & SCE_CTRL_LTRIGGER) && (keys&SCE_CTRL_CROSS) && tmp == 0 && ligneOption == 3) {
                mode = 4;
                gpJeu->getGenerique()->initSelection();
            ...
            if ((keys&SCE_CTRL_LEFT) && !tmp) {
                ...
                if (ligneOption == 1) {
					...
					gpJeu->getAudio()->playSound(3);}
                if (ligneOption == 2) {
                    if (getLanguage()==4) setLanguage(2); 
					else setLanguage(getLanguage()-1);
					tmp=1;
                    gpJeu->getAudio()->playSound(3);}}
            if ((keys&SCE_CTRL_RIGHT) && !tmp) {
                if (ligneOption == 0) {
					int vtemp = volume;
                ...
                if (ligneOption == 2) {
					if (getLanguage()==2) setLanguage(4); 
					else setLanguage(getLanguage()+1);
					tmp=1;
                    gpJeu->getAudio()->playSound(3);}}
        ...
        case 7 :
            ...
            if (!(keys&SCE_CTRL_CROSS) && !(keys&SCE_CTRL_UP) && !(keys&SCE_CTRL_DOWN) && tmp) tmp=0;
            break;
        case 8 :
            if (!(keys & SCE_CTRL_LTRIGGER) && (keys&SCE_CTRL_CROSS) && tmp == 0) {
                int old;
            ...
            if (!(keys&SCE_CTRL_CROSS) && tmp) tmp=0;
            break;
        case 9 :
            if (!(keys & SCE_CTRL_LTRIGGER) && (keys&SCE_CTRL_CROSS) && tmp == 0) {
                if (ligneVal==0) {
            ...
            if (((keys&SCE_CTRL_UP) || (keys&SCE_CTRL_DOWN)) && !tmp) {
        ...
        case 11 :
            ...
            break;
        case 12 :
            if (!(keys & SCE_CTRL_LTRIGGER) && (keys&SCE_CTRL_CROSS) && !tmp) {
                mode = 0;
				gpJeu->setStop(false);
                gpJeu->getAudio()->playSound(2);
                tmp=1;
        ...
        case 13 :
            ...
            if (!(keys&SCE_CTRL_CROSS) && !(keys&SCE_CTRL_LEFT) && !(keys&SCE_CTRL_RIGHT) && tmp) tmp=0;
            break;
        case 14 :
            if ((keys&SCE_CTRL_CROSS) && tmp == 0) {
        ...
        case 15 :
            ...
            if (((keys&SCE_CTRL_LEFT) || (keys&SCE_CTRL_RIGHT)) && ligneRecord==3 && !tmp) {
        ...
        case 17 :
           ...
           if (!(keys&SCE_CTRL_CROSS) && !(keys&SCE_CTRL_RIGHT) && tmp) tmp=0;
             break;
        case 18 :
            if ((keys&SCE_CTRL_CROSS) && !tmp) {
                mode = 0;
				gpJeu->setStop(false);
                gpJeu->getAudio()->playSound(2);
                tmp=1;
            ...
            if (!(keys&SCE_CTRL_CROSS) && !(keys&SCE_CTRL_LEFT) && tmp) tmp=0;
            break;
        case 19 :
        case 20 :
        case 21 :
        ...
        case 22 :
            ...
            || (keys&0) || (keys&0)
                ...
                if ((keys&0) || (keys&0))telep=0;
                if ((keys&0) || (keys&0))telep=1;
                if ((keys&0) || (keys&0))telep=2;
                if ((keys&0) || (keys&0))telep=3;
                if ((keys&0) || (keys&0))telep=4;
                if ((keys&0) || (keys&0))telep=5;
                if ((keys&0) || (keys&0))telep=6;
                if ((keys&0) || (keys&0))telep=7;
                if ((keys&0) || (keys&0))telep=8;
                gpJeu->getAudio()->playSound(3); tmp=1;
            }
            
            if (!(keys&SCE_CTRL_CROSS) && !(keys&SCE_CTRL_LEFT) && !(keys&SCE_CTRL_RIGHT) 
